...
rule_match_migrations=$(uci -q show nikki | sed -n 's/^nikki\.@rule\[\([[:digit:]]\+\)\]\.match=.*/rename nikki.@rule[\1].match=matcher/p')
[ -z "$rule_match_migrations" ] || printf '%s\n' "$rule_match_migrations" | uci batch
